...
function CardModal1({modalOpen, closeModal, modalOpenerRef}) {
  return ( 
    modalOpen?

    <ModalTemplate coverExist={true} exitByOuterClick={false} modalOpenerRef={modalOpenerRef} isModalOpened={modalOpen}>
      <Card $width='500px' $height='300px'>
        <div>닫기와 확인 버튼 2개가 있고, 외부 영역을 눌러도 모달이 닫히지 않아요.</div>
        <RowDirectionContainer $gap='5px' $position='absolute' $bottom='12px' $right='12px'>
